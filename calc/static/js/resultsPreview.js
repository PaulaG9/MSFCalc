...
function getPreview(){
    ...
    var formInst=$('[id^=num_]')
    
    for (var x=1; x<formInst.length+1; x++){
        var numpatients=parseInt($('#num_patients_' +x).val()); 
        console.log(numpatients);
        var duration=parseInt($('#duration_' +x).val()); 
        console.log(duration);
        var monincrease=parseInt($('#monthly_increase_' +x).val()); 
        ...
        for (var i=1; i<results.length+1; i++){ 
            ...
            var frequency=parseInt($('#frequency_'+ x +'_' + msfcode).val()); 
